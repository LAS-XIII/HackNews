...
async function callData() {
    ...
    arrayofThings.forEach((data) => {
        let timestamp = _.get(data, "time", "data non trovata");
        let formattedDate = formatTimestamp(timestamp * 1000);
        testo +=
        `
            <div class="card">
                <h2 class="title cardElement">${_.get(data, "title", "titolo non trovato")}</h2>
                <p class="time cardElement">${formattedDate}</p>
                <a class="description cardElement" target="blank" href="${_.get(data, "url", "")}" >${_.get(data, "url", "")!="" ? "Click the link for more information" : "link non disponibile"}</a>
            </div>
        `;
        array.shift()
    })
